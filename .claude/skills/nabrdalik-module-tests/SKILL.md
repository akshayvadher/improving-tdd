...
- [ ] When the facade calls a **binary-payload gateway** (file storage), keep idempotency in the gateway (`put` returns `{ alreadyExisted }`) and validation in the module's schema (magic-byte sniff, size cap, content hash in `parse<Module>Upload`). The in-memory adapter defensive-copies on both write and read so callers cannot poke the store through returned bytes. Counting side-effects (`putCallCount === 0` after auth/validation failure) is *invariant pinning*, not call-count coupling — assert "no side effect on error path," never "method X was called N times." Canonical example: `apps/library/src/shared/file-storage-gateway/` + `ThrowingOnceFileStorageGateway` and `CountingFileStorageGateway` in `catalog.facade.spec.ts`. Detailed teaching: `GUIDE.md` — Binary-payload gateways.
